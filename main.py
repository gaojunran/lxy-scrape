...
def select(url: str, selector: str, single: bool = False, text: bool = False) -> list[Tag] | Tag:
...
# 截止日期和联系人需要从正文中抓取
def scrape_page(url: str, year: int) -> tuple[datetime, str]:
    texts = select(url, '.xwnr_content', single=True).get_text()
    due_date = re.search(r'截止(?:(?:日期)|(?:时间))：(.+?日)', texts)
    if due_date:
        due_date = due_date.group(1)
        try:
            due_date = datetime.strptime(due_date, '%Y年%m月%d日')
        except ValueError:
            try:
                due_date = datetime.strptime(due_date, '%m月%d日').replace(year=year)
            except ValueError:
                due_date = ''
    else:
        due_date = ''
...
def scrape_list(urls: str) -> list[AnnounceRecord]:
    output_rows = [['项目名称', '项目类型', '发布日期', '截止日期', '联系人', '网页URL']]
    for url in urls:
        items = select(url, '.xw-list ul li a')
        for idx, item in enumerate(items):
            ...
            obj = AnnounceRecord(name=title, type=type, post_date=post_date, due_date=due_date, contact=contact, url=url)
            output_rows.append(obj.to_csv_row())
    return output_rows

def to_csv(data: list[list]) -> None:
    with open("output.csv", mode='w', newline='', encoding='utf-8') as file:
        writer = csv.writer(file)
        writer.writerows(data)
    print(f"CSV 文件已写入！")

def to_db(data: list[list]) -> None:
    headers = data[0]
    rows = data[1:]
